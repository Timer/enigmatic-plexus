...
function build_graph(genes, relations, groups) {
  for (var k in relations) {
    if (!relations.hasOwnProperty(k)) continue;
    var gene = _.findWhere(genes, { id: relations[k]['$']['entry1'] });
    var group;
    if (gene != null) {
      gene['edges'].push(relations[k]['$']['entry2']);
    } else if (group = _.findWhere(groups, { '$': { id: relations[k]['$']['entry1'] } })) {
      console.log(group);
    }
  }
}
